...
dataset = pd.read_csv('./actionserver/products.csv')
...
class ActionShowproduct(Action):
    ...
    def run(
        ...
        for i in data['frendy']['product_imgs']:
                url = str(i)
                dispatcher.utter_message("product of that seller is ")
...
class OrderForm(FormAction):
    ...
    def slot_mappings(self) -> Dict[Text, Union[Dict, List[Dict]]]:
        return {"product_name": self.from_entity("any_thing"),"quantity": self.from_entity("quantity"),"proceed": self.from_intent("inform")}
    ...
    def submit(
        ...
        for x in range(len(product_list)):
            dispatcher.utter_message("{} : {} : {}".format(product_list[x],quant_list[x],dataset[product_list[x]][0]))
            z = int(dataset[product_list[x]][0])
            amount += z
